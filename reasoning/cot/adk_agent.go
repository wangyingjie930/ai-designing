...
// ADKAgent 把 CoT verifier 核心逻辑包装成 Eino ADK Agent。
type ADKAgent struct {
	core *ReasoningAgent
}

// NewADKAgent 创建 ADK 包装层，避免核心推理逻辑依赖 runner 细节。
func NewADKAgent(core *ReasoningAgent) (*ADKAgent, error) {
	if core == nil {
		return nil, errors.New("cot reasoning agent is required")
	}
	return &ADKAgent{core: core}, nil
}

// NewRunner 直接创建 CoT Agent 和 ADK Runner，适合命令入口和业务入口复用。
...
// Run 实现 Eino ADK Agent 接口，把输入消息交给 CoT verifier 并发出最终 assistant message。
func (a *ADKAgent) Run(ctx context.Context, input *adk.AgentInput, _ ...adk.AgentRunOption) *adk.AsyncIterator[*adk.AgentEvent] {
	...
	go func() {
		defer gen.Close()
		if a == nil || a.core == nil {
			gen.Send(&adk.AgentEvent{Err: errors.New("cot reasoning agent is required")})
			return
		}
		...
		response, err := a.core.Solve(ctx, Request{Messages: messages})
		if err != nil {
