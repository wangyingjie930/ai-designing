...
5. 每轮回答完成后以 fire-and-forget 方式提交对话快照，不阻塞主回答返回。
6. 后台最多运行一个提取任务；忙时只保留最新快照，当前任务完成后执行一次 trailing extraction。
7. 下一轮先读取两个索引，再由召回模型选择最多 5 个相关主题文件。
8. 读取选中内容并作为独立 memory context 注入主 Agent。
9. 路径越界防护、team 写入敏感信息防护和原子文件写入。
10. 一个真实多轮 CLI 和一组不联网的确定性测试。
...
主回答生成后，`Runner` 先把回答交还调用方，再由 `ExtractionScheduler.Schedule` 以 fire-and-forget 方式提交完整对话快照。后台 `Extractor.Extract` 使用独立模型调用处理新增消息：
...
调度器遵循 Claude Code 的并发语义：

1. `Schedule` 只复制快照并立即返回，不等待模型或文件写入。
2. 同一时间最多运行一个提取任务，避免游标和索引并发写入。
3. 提取进行中收到新快照时，用最新快照覆盖旧 pending；当前任务完成后只执行一次 trailing extraction。
4. `Drain` 等待当前任务及 trailing extraction，用于测试和进程退出；交互主链路不调用它。

三轮脚本 CLI 会先打印主回答，再调用带 60 秒软超时的 `Drain`，然后打印 `written` 和 warning 并进入下一轮。这对应 Claude Code headless 模式“答案已经 flush，只延迟退出/后续脚本步骤”，不把抽取延迟算到主回答返回时间里。
...
- 提取模型失败：已经生成的主回答仍然有效；错误由后台结果记录，CLI 在 Drain 后输出记忆维护警告。
- Drain 超时：不取消仍在运行的后台提取，也不把主回答改成失败；脚本记录 warning 后继续或退出。
...
- 顺序严格为 `Recall -> Main Agent -> Schedule`，其中 `Schedule` 不等待 Extract。
- 阻塞提取 fake 已启动但未释放时，`RunTurn` 仍能立即返回主回答。
- 同一时间只运行一个提取，连续 Schedule 只保留最新 pending 快照并执行一次 trailing extraction。
- Drain 会等待当前任务和 trailing extraction，并返回所有后台写入与 warning。
