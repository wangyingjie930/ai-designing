...
Planner 只注册 `update_profile`、`update_todo` 两个写工具；它们直接提交且返回 committed version。`ProposedActions` 只承载转人工、停止触达等尚未执行的动作。
...
- `Type`：转人工、停止触达、创建 Outbox 等非 Planner 直接写动作；
- 结构化 Payload；
- RiskLevel；
- 预期状态版本；
- Policy Evidence。
...
Profile/TODO 不走 ProposedAction；它们由 Planner Tool 在规划阶段直接提交。
...
| 角色 | 工具 |
| --- | --- |
| Planner | `update_profile`、`update_todo` |
| PlanPolicyGate | 无 |
...
每个 Planner 写工具在业务数据事务内同步写 Tool Ledger；Orchestrator 随后保存 Tool Result 和 committed version。Checkpoint 保存初始 Snapshot、写后重载的 Profile/TODO、Tool Ledger 引用、Manifest、Plan、Verdict、Draft、Bubbles、ProposedActions、rewrite 次数、阶段版本、状态和 heartbeat；不保存 Chain-of-Thought。
...
Planner 阶段可能已经通过 `update_profile`、`update_todo` 提交 Profile/TODO；这些写入不会因后续 Reviewer、Thinker 或 Writer 失败而回滚。Final Review 通过后，CommitCoordinator 只处理尚未执行的外部动作和最终回复：

1. OwnershipGate 校验 Run 仍拥有当前 Session；
2. 校验当前 Profile/TODO 版本与 Planner Tool Result 中的 committed version 一致；
3. CommitCoordinator 在同一事务中执行转人工、停止触达等未提交 ProposedActions；
4. 同一事务写 ActionAudit 和最终回复 OutboxMessage；
5. DeliveryWorker 使用稳定 MessageID 幂等发送；
6. 发送失败只重试 Outbox，不重新运行 Agent。

相同 `RunID + ActionID` 只能提交一次。Planner Tool Write 使用 MessageID 级 Tool Ledger 防重放；最终 Ownership/CAS 冲突把 Run 标记 PREEMPTED，不发送旧回复，但不回滚此前已经成功且幂等提交的 Profile/TODO 更新。
...
- Read-only Tool 错误可降级；Planner 写 Tool 不在 Executor 内盲目重试，模型重试或恢复时必须先查询 Tool Ledger；转人工、停止触达等其他写动作只在 Commit 中通过幂等事务执行。
...
4. Reviewer block 时不产生最终回复 Outbox 或其他未提交动作，但不回滚 Planner 已成功提交的 Profile/TODO；transfer 只通过 CommitCoordinator 提交一次 TransferToHuman 和人工通知 Outbox；
