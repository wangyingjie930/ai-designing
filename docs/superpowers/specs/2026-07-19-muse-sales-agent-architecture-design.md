...
6. 默认测试不访问真实模型或网络；
7. `update_profile` 和 `update_todo` 保留为核心业务能力，但从模型即时写工具改为可验证、可恢复、最终统一提交的领域动作。
...
- 证据驱动的 ProfilePatch 和受状态机约束的 TodoTransition；
...
RunService / cmd/muse-agent
  -> Eino ADK Runner + CheckPointStore + CozeLoop
      -> MuseAgent（确定性 Orchestrator）
          ...
          -> RawInputGuard
          -> Planner
          -> ActionValidator（ProfilePatch / TodoTransition）
          -> ReviewRouter
...
### 5.5 ProfilePatch：保留 update_profile 业务能力

`silicon-sage` 的 Planner 会从对话中补充用户称呼、职业、需求场景、目标、痛点、紧迫度、截止时间、当前水平、薄弱项、目标水平、考试历史、每日学习投入、偏好时段、设备、当前 SKU、线索来源和是否入群。`muse` 保留这些业务字段，但不允许模型提交整个 Profile 或直接覆盖数据库。
...
- 同一 Patch 不允许重复字段；
- Commit 时按字段合并并检查 Profile 版本，不进行全量覆盖。

证据充分、字段合法的常规 ProfilePatch 由 ActionValidator 直接通过，不触发 PlanPolicyGate；缺少证据或字段越权直接拒绝，语义存在歧义时才进入 PlanPolicyGate。

### 5.6 TodoTransition：保留 update_todo 业务能力

`silicon-sage` 的 TODO 由 `phase/stage/task/step/status` 和追加日志组成。`muse` 不允许 Planner 任意覆盖这些字段，而是输出从当前节点到目标节点的 Transition：
...
服务端 `TodoWorkflow` 从冻结的 TODO Skill/SOP 编译出允许的节点和边。ActionValidator 必须校验：
...
- TODO 更新和由此产生的规则任务写入同一事务/Outbox，不启动脱离 Run 的后台 goroutine。

同阶段、合法且证据充分的 TodoTransition 不触发 PlanPolicyGate。非法跳转由确定性校验器直接拒绝；是否满足业务里程碑存在语义歧义时才调用 PlanPolicyGate，Reviewer 也不能绕过状态机硬约束。
...
当用户对话产生新的画像事实或业务进度时，Planner 应分别输出 `UpdateProfile(ProfilePatch)`、`AdvanceTodo(TodoTransition)` ProposedAction。没有证据时不产生 Patch；没有合法状态变化时不产生 Transition。

Planner 输出非法时只重试 Planner 一次。重试前不得执行任何业务副作用。

### 6.3 ReviewRouter

Plan Policy Gate 不是常态化调用。ActionValidator 先校验 ProfilePatch 和 TodoTransition 的硬约束，ReviewRouter 再使用确定性字段判断是否需要语义审核。

以下任一条件触发 Plan Policy Gate：

- ProposedAction 包含用户可见外部动作、定时触达或人工接管；
- ProfilePatch 的证据存在语义歧义，或包含允许推断但风险较高的字段；
- TodoTransition 的节点合法，但是否满足业务里程碑需要语义判断；
- Plan 包含价格、名额、交付或服务承诺；
- 原始输入包含拒绝信号，但 Plan 仍继续营销；
- RiskLevel 非 low；
- Plan 存在不确定项或合约冲突。

普通信息咨询，以及证据充分的常规 ProfilePatch、合法同阶段 TodoTransition，在无其他风险信号时跳过。字段越权、无证据事实和非法 TODO 跳转由 ActionValidator 直接拒绝，不交给模型裁决硬约束。
...
`update_profile` 和 `update_todo` 在 `muse` 中不再作为模型可直接调用的 Tool 暴露。它们分别成为 `UpdateProfile(ProfilePatch)` 和 `AdvanceTodo(TodoTransition)` ProposedAction：Planner 负责提出，ActionValidator 负责校验，CommitCoordinator 负责最终执行。这样保留业务能力，同时消除生成阶段即时写库和重试重放。
...
4. ProfilePatch 按字段合并并 CAS 更新 Profile；TodoTransition 按状态机边执行、CAS 更新 TODO 并追加 TodoLog；
5. TODO Transition 触发的后续规则任务与 ActionAudit、OutboxMessage 在同一事务中落盘；
6. DeliveryWorker 使用稳定 MessageID 幂等发送；
7. 发送失败只重试 Outbox，不重新运行 Agent。
...
16. 用户明确提供职业、目标等画像事实时生成 ProfilePatch，低风险且证据充分的 Patch 不调用 PlanPolicyGate，最终只提交一次；
17. Planner 推测未被用户确认的 SKU、考试成绩或入群状态时，ActionValidator 拒绝 ProfilePatch；
18. 合法同阶段 TodoTransition 不调用 PlanPolicyGate，Commit 后追加一条 TodoLog；
19. TODO 跨阶段跳转、from 版本不一致或目标节点不在 SOP 时，ActionValidator 在模型审核前直接拒绝；
20. FinalReviewer block、CAS 冲突或恢复重放时，ProfilePatch 和 TodoTransition 均不得提前或重复提交。
